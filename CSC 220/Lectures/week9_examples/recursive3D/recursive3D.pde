/************************************************************
/* Sketch: CSC220Fall2022Recur3D, derived from CSC220Fall2020Recur3D,
...
void setup() {
  ...
  STUDENTIMAGE = loadImage("Hex306090B.png");
...
void drawShape(int mydepth, final float rotation) {
  ...
  // You MUST use some 2D shapes and MAY use 3D shapes, with copies of at least one 2D shape
  // rotated in X, Y, and Z directions, and
  // nested to the degree given by global recursionDepth+1. 
  
  /*
    STUDENT B REVISION:
    You do NOT need to texture a STUDENTSHAPE with the STUDENTIMAGE. That works only for a
    subset of PShapes, notably RECT PShapes. 
    
    Instead, plot via shape(STUDENTSHAPE, 0, 0, yourwidth, yourheight) one library-supplied PShape
    built with STUDENTSHAPE  = createShape(Ã¢â‚¬Â¦) (could be a 2D vector file via loadShape() if you
    know how to build those), AND display one STUDENTIMAGE loaded with loadImage() and plotted
    with image(STUDENTIMAGE, 0, 0, yourwidth, yourheight). 
    
    Call createShape() and loadImage() up in setup() to avoid the overhead of creating them
    in each call to drawShape().

    ALSO you do NOT need to use vertex() in creating your PShape.
    You can use a canned library shape as documented in the createShape() documentation.
  */
    ...
    if (! (controlKey || altKey)) {
      ...
      if (controlKey) {
        if (controlKey) {
          ...
          // and (int(key) == 24 && ! shiftKey) is how Windows detects control keys.
