...
void setup() {
    ...
    customPImage = loadImage("woodTexture.jpg"); // load before makeCustomPShape(), USE YOUR OWN
    ...
    avatars[0] = new MyCloset(width/4, height/4, 0, 4, 4, 3, 8);
    ...
    avatars[5] = new MyCloset(3*width/4, 3*height/4, 0, 3, 3, 3, 4);
    ...
    avatars[12] = new VectorAvatar(int(random(0,width)), int(random(0,height)), 0, 5, 6, round(random(5,7)), 1.5, customPImage);
    for (int i = 13 ; i < avatars.length-1 ; i++) { // STUDENT BE CAREFUL TO START i AT *YOUR* NEXT OPEN SLOT!
        int zspeed = round(random(-10,10));
        if (zspeed == 0) { zspeed = 1 ; }
        avatars[i] = new MyCloset(int(random(0,width)), int(random(0, height)),
                int(random(minimumZ, maximumZ))/2,
                round(random(-5,5)), round(random(-5,4)), zspeed, .25);
    }
