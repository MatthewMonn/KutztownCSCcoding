...
/*    under usexxxxr control, and indirected recursion to drawIndirectRecursiveShape()
...
final int strokeSize = 5 ; // basis for strokeWeight()
...
PShape MERCURY,EARTH,MOON,MARS,SUN; //New PShape for drawing Planets 
PImage STUDENTIMAGE2;  //Image for Milky Way. 
void setup() {
  ...
  STUDENTIMAGE = loadImage("Stars.jpg"); //Link: https://stock.adobe.com/search?k=galaxy+stars&asset_id=370356524
  STUDENTIMAGE2 = loadImage("MILKYWAY.jpg"); // Link: https://www.discovermagazine.com/the-sciences/10-facts-you-may-not-know-about-the-milky-way
  STUDENTIMAGE.resize(width,height); //resizes image for size of window. 
  // STUDENT replace with your own image file for drawShape()
  background(STUDENTIMAGE); //reset background for stars for theme of project.
  ...
  push(); 
  fill(color(60,99,100)); //sets the "sun" as STUDENTSHAPE that is also nested. 
  STUDENTSHAPE  = createShape(ELLIPSE, 0, 0, 400, 400); //creation of STUDENTSHAPE. 
  pop(); 
  push(); 
  fill(color(0,1,71)); //color of Mercury. 
  MERCURY = createShape(ELLIPSE, 0, 0, 200, 200); //creation of each shape is ellipse for planets. 
  fill(color(214,74,75)); //color for Earth.
  EARTH = createShape(ELLIPSE, 0, 0, 400, 400); 
  fill(color(0,1,71)); //color for Moon. 
  MOON = createShape(ELLIPSE, 0, 0, 150, 150); 
  fill(color(30,84,92)); //color for Mars. 
  MARS = createShape(ELLIPSE, 0, 0, 400, 400); 
  pop(); 
...
void draw() {
  ...
  if (! isPaint) {
    background(STUDENTIMAGE); //0,100,50 was original, set to STUDENTIMAGE for stars. 
  }
  push();
...
void drawRecursiveShape(final int mydepth, final int recursionDepth, final float rotation) {
  if (mydepth >= recursionDepth) {
    ...
    push(); //new code for drawing new shape and rotation, keeps drawn shape when recurison is increased.  
    helpRotate(); 
    drawShape(mydepth, rotation); 
    pop();
  }
...
void drawShape(int mydepth, final float rotation) {
 ...
 push();  
 for (mydepth = 0; mydepth <= recursionDepth; mydepth+=1) { //Nesting loop for STUDENTSHAPE, nests the Sun in image. 
   scale(1.0/(mydepth +1)); //rescales each time. 
  shape(STUDENTSHAPE,250,250,500,500); // creation of Sun, starts on Z plane. 
  image(STUDENTIMAGE2, -1500,500,width, height); //Milkyway image creation. 
  shape(MERCURY,700,125,200,200); //creation of each planet below in respective locations. 
  shape(EARTH,1300,250,400,400); 
  shape(MOON,1400,0,100,100); 
  shape(MARS,1800,250,300,300); 
  push(); 
  rotateY(radians(70.0)); //nests the Sun on Y plane. 
  shape(STUDENTSHAPE,225,250,500,500);
  pop(); 
  push(); 
  rotateX(radians(70.0)); //nests the Sun on X plane. 
  shape(STUDENTSHAPE,250,225,500,500);
  pop(); 
  } 
 pop();
}
...
void keyPressed() {
  ...
  if (key == '2') { //new creation of keys 2,4,6,8 for different spanning levels. 
    spanSteps = 2;
  } else if (key == '4') {
    spanSteps = 4;
  } else if (key == '6') {
...
void moveCameraRotateWorldKeys() {
  if (keyPressed) {
    if (! (controlKey || altKey)) {
      ...
      if (controlKey) { //implementation of controlkey to either increment or decrement x,y,z SpanLimits. 
        ...
        if (key == 'x' || ((int(key) == 24 || (key == CODED && keyCode == 88)) && ! shiftKey)) {
          xSpanLimit = constrain(xSpanLimit + spanIncr, -xSpanRange, xSpanRange);
        } else if (key == 'y'|| ((int(key) == 25 || (key == CODED && keyCode == 89)) && ! shiftKey)) {
         ySpanLimit = constrain(ySpanLimit + spanIncr, -ySpanRange, ySpanRange);
        } else if (key == 'z'|| ((int(key) == 26 || (key == CODED && keyCode == 90)) && ! shiftKey)) {
          zSpanLimit = constrain(zSpanLimit + spanIncr, -zSpanRange, zSpanRange);
      ...
      if (altKey) { //altKey section to increment spanIncr or decrement that changes the x,y,z ScaleAmount.  
        if (key == 'x') {
           xScaleAmount = constrain(xScaleAmount + spanIncr, -xSpanRange, xSpanRange);
        } else if (key == 'y') {
          yScaleAmount = constrain(yScaleAmount + spanIncr, -ySpanRange, ySpanRange);
        } else if (key == 'z') {
          zScaleAmount = constrain(zScaleAmount + spanIncr, -zSpanRange, zSpanRange);
        } else if (key == 'X') {
          xScaleAmount = constrain(xScaleAmount - spanIncr, -xSpanRange, xSpanRange);
        } else if (key == 'Y') {
          yScaleAmount = constrain(yScaleAmount - spanIncr, -ySpanRange, ySpanRange);
