...
void makeglobePShape() {
  ...
  globePImage = loadImage("7Ball.jpg"); //link for image: https://www.sharecg.com/v/12972/Texture/Billiard-Ball---N+-7
...
class Globey implements Avatar {
  ...
  void display() {
    push();
    translate(x, y, z);
    if (mowCounter > 0) { //makes the "imploding globey". 
      scale(mowScaleXYZ);
      mowScaleXYZ -= 0.005; //decrement 0.005 starting from 1.0 for each globey hit if mow occurs. 
    }
  ...
  void mow() {
    mowCounter += 1 ; //increases the mowCounter.
  }
}
