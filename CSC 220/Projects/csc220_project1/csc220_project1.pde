/************************************************************
/* Authors: Dr. Parson & (Matthew Monn)
...
void setup() {
  ...
  size(2000, 1000); // YOU may change size.
  ...
  avatars = new Avatar [51] ;
  avatars[0] = new PokeMMOLogo(width/4, height/4, 2, 3, 2, 0, 0, .5);
  ...
  // size.
  int RanColor = color(random(0,255), random(0,255), random(0,255)); //gives a random color when used.
  avatars[1] = new Furniture(width/2, 25, width, 50, RanColor);  // 10 pixels wide boundary is impenetrable
  avatars[2] = new Furniture(width/2, 975, width, 50, RanColor);
  avatars[3] = new Furniture(5, height/2, 10, height, RanColor);
  avatars[4] = new Furniture(width-5, height/2, 25, height, RanColor);
  avatars[5] = new PokeMMOLogo(3*width/4, 3*height/4, 2, 1, 1, 0, 0, .5);
  int magenta = color(255, 0, 255, 255);
  final int barlength = 200 ;
  avatars[6] = new Furniture(barlength/2, height/2, barlength, 50, RanColor); 
  avatars[7] = new Furniture(width-barlength/2, 2*height/3, barlength, 75, RanColor);
  avatars[8] = new Furniture(width/3, barlength/2, 10, barlength, RanColor);
  avatars[9] = new Furniture(width/3, height-barlength/2, 100, barlength, RanColor);
  color orange = color(255,184,0,255);
  avatars[10] = new Paddle(width/2, height/2, barlength, 7, .5, cyan); //moves paddle in clock wise rotation.
  avatars[11] = new Paddle2(width/4, height/3, 300, 25, 1, magenta); //moves paddle2 counter clock wise rotation.
  avatars[12] = new Furniture(width/2, 250, 300, 300, orange); //big orange sphere furniture to take space.
for (int i = 13 ; i < avatars.length ; i++) {
   avatars[i] = new PokeMMOLogo(int(random(0,width)), int(random(0, height)),
      round(random(1,5)), round(random(-5,-1)), .25,  0, 0, 1);
  }
